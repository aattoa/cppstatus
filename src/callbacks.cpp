...
namespace {
    constexpr std::size_t max_bar_size = 256;

    constexpr std::string_view failure_symbol = "(?)";

    auto execute_command(std::span<char, max_bar_size> const output, char const* const command)
        -> bool
    {
...
auto cppstatus::callbacks::file_content(Block::Out out, char const* const path) -> void
{
    ...
    for (std::size_t i = 0; i != max_bar_size; ++i) {
